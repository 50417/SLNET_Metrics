classdef model_metric < handle
    ...
    properties
        ...
        blk_info;
        lvl_info;
        blk_count_old; % Block Count based on C-corpus custom tool
...
%                 fprintf('Found %d blocks\n', count);
        ...
        function correlation_analysis(obj, flag , varargin)
            ...
            if flag
                ...
                innerquery = ['select b.*, a.cnt from  ',varargin{1}, '_Metric b join ( select File_id, Model_Name, count(blk_type) as cnt from ',varargin{1}, ...
                    '_Block_Info group by File_id,Model_Name) a on a.File_id = b.File_id and  a.Model_Name = b.Model_Name'];
           if nargin > 1
                for i = 2:nargin-2
                    innerquery = [innerquery,' union all select b.*, a.cnt from  ',varargin{i}, '_Metric b join ( select File_id, Model_Name, count(blk_type) as cnt from ',varargin{i}, ...
                    '_Block_Info group by File_id,Model_Name) a on a.File_id = b.File_id and  a.Model_Name = b.Model_Name'];
        ...
        function analyze_metrics(obj,flag)
            ...
            if flag
                ...
                ' not in (select file_id,Model_Name from GitHub_Metric where is_lib=1) and substr(Model_Name,0,length(Model_name)-3) IN (' original_mdl_name{1} ') ',...
                ' union all',...
                ' select * from MATC_Subsys_Info where (file_id,Model_Name)', ...
                'not in (select file_id,Model_Name from matc_Metric where is_lib=1) and substr(Model_Name,0,length(Model_name)-3) IN (' original_mdl_name{1} ')',...
                 ' union all',...
            ...
            query_matc_models = 'select avg(SCHK_Block_count) from MATC_Metric where is_lib=0 and is_test = -1 ';
            ...
            if (flag)
                models_use_mdlref_query = ['select mdlref_nam_count from',...
                                ' (select * from github_metric where unique_mdl_ref_count>0 and  is_lib=0 and is_test = -1 and substr(Model_Name,0,length(Model_name)-3) IN (' original_mdl_name{1} ')',...
                                 '   union all',...
                                  '  select * from  matc_metric where unique_mdl_ref_count>0 and is_lib=0 and is_test = -1 and substr(Model_Name,0,length(Model_name)-3) IN (' original_mdl_name{1} ')',...
                                  '   union all',...
            ...
            models_use_mdlref_query = ['select mdlref_nam_count from',...
                                ' (select * from github_metric where unique_mdl_ref_count>0 and  is_lib=0 and is_test = -1 ',...
                                 '   union all',...
                                  '  select * from  matc_metric where unique_mdl_ref_count>0 and is_lib=0 and is_test = -1 ',...
                                   ' )'];
            end
            ...
            if flag
                models_use_sfun_query = ['select sfun_nam_count from',...
                                ' (select * from github_metric where unique_sfun_count>0 and  is_lib=0 and is_test = -1 and substr(Model_Name,0,length(Model_name)-3) IN (' original_mdl_name{1} ')',...
                                 '   union all',...
                                  '  select * from  matc_metric where unique_sfun_count>0 and is_lib=0 and is_test = -1 and substr(Model_Name,0,length(Model_name)-3) IN (' original_mdl_name{1} ')',...
                                   '   union all',...
            ...
            models_use_sfun_query = ['select sfun_nam_count from',...
                                ' (select * from github_metric where unique_sfun_count>0 and  is_lib=0 and is_test = -1 ',...
                                 '   union all',...
                                  '  select * from  matc_metric where unique_sfun_count>0 and is_lib=0 and is_test = -1 ',...
                                   ' )'];
            end
        ...
        function res = total_analyze_metric(obj,choice,original_flag)
            ...
            Example : 
            obj.total_analyze_metric('Github_metric',true)
            ...
            %}
                    ...
                    switch lower(choice)
                        case 'github_metric'
                             if original_flag
                                 original_mdl_name = obj.list_of_model_name();
                                 most_frequentlused_blocks_query = strcat('select BLK_TYPE,sum(count)  as c from GitHub_Block_Info  where (File_id,Model_Name) in ( ',...
                                     ' select File_id,Model_Name from GitHub_Metric where is_Lib=0 and is_test = -1  and substr(Model_Name,0,length(Model_name)-3) IN (',...
                                     original_mdl_name,') )',...
                                     ' group by BLK_TYPE order by  c desc');
                                 total_hidden_conn_query = strcat('select sum(Conn_count_hidden_only) from GitHub_Hierar_Info where (File_id,Model_Name) in (SELECT distinct File_id,Model_Name from ',...
                                 ...
                                 most_frequentlused_blocks_query = ['select BLK_TYPE,sum(count)  as c from GitHub_Block_Info  where (File_id,Model_Name) in ( ',...
                                     ' select File_id,Model_Name from GitHub_Metric where is_Lib=0 and is_test = -1  )',...
                                     ' group by BLK_TYPE order by  c desc'];
                                  total_hidden_conn_query = ['select sum(Conn_count_hidden_only) from GitHub_Hierar_Info where (File_id,Model_Name) in (SELECT distinct File_id,Model_Name from ',choice,'   where is_Lib = 0 and is_test = -1  )  '];
                            end
                        case 'matc_metric'
                            if original_flag
                             original_mdl_name = obj.list_of_model_name();
                                 most_frequentlused_blocks_query = strcat('select BLK_TYPE,sum(count)  as c from matc_Block_Info  where (File_id,Model_Name) in ( ',...
                                     ' select File_id,Model_Name from matc_Metric where is_Lib=0 and is_test = -1  and substr(Model_Name,0,length(Model_name)-3) IN (',...
                                     original_mdl_name,') )',...
                                     ' group by BLK_TYPE order by  c desc');
                                 total_hidden_conn_query = strcat('select sum(Conn_count_hidden_only) from matc_Hierar_Info where (File_id,Model_Name) in (SELECT distinct File_id,Model_Name from ',...
                                     {' '},choice,{' '},'   where is_Lib = 0 and is_test = -1',...
                                     {' '},' and substr(Model_Name,0,length(Model_name)-3) IN (',original_mdl_name,') )  ');
                            else
                                most_frequentlused_blocks_query = ['select BLK_TYPE,sum(count)  as c from Matc_Block_Info where (File_id,Model_Name) in ( ',...
                                 ' select File_id,Model_Name from matc_Metric where is_Lib=0 and is_test = -1 )',...
                                 '  group by BLK_TYPE order by  c desc'];
                              total_hidden_conn_query = ['select sum(Conn_count_hidden_only) from matc_Hierar_Info where (File_id,Model_Name) in (SELECT distinct File_id,Model_Name from ',choice,'   where is_Lib = 0 and is_test = -1  )  '];
                            end
                        case 'sourceforge_metric'
                            if original_flag
                            original_mdl_name = obj.list_of_model_name();
                                 most_frequentlused_blocks_query = strcat('select BLK_TYPE,sum(count)  as c from sourceforge_Block_Info  where (File_id,Model_Name) in ( ',...
                                     ' select File_id,Model_Name from sourceforge_Metric where is_Lib=0 and is_test = -1  and substr(Model_Name,0,length(Model_name)-3) IN (',...
                                     original_mdl_name,') )',...
                                     ' group by BLK_TYPE order by  c desc');
                       ...
                       case 'tutorial_metric'
                           if original_flag
                               
                             original_mdl_name = obj.list_of_model_name();
                                 most_frequentlused_blocks_query = strcat('select BLK_TYPE,sum(count)  as c from tutorial_Block_Info  where (File_id,Model_Name) in ( ',...
                                     ' select File_id,Model_Name from tutorial_Metric where is_Lib=0 and is_test = -1  and substr(Model_Name,0,length(Model_name)-3) IN (',...
                                     original_mdl_name,') )',...
                                     ' group by BLK_TYPE order by  c desc');
                                 total_hidden_conn_query = strcat('select sum(Conn_count_hidden_only) from tutorial_Hierar_Info where (File_id,Model_Name) in (SELECT distinct File_id,Model_Name from ',...
                                     {' '},choice,{' '},'   where is_Lib = 0 and is_test = -1',...
                                     {' '},' and substr(Model_Name,0,length(Model_name)-3) IN (',original_mdl_name,') )  ');
                           else 
                       ...
                       case 'others_metric'
                           if original_flag
                            original_mdl_name = obj.list_of_model_name();
                                 most_frequentlused_blocks_query = strcat('select BLK_TYPE,sum(count)  as c from others_Block_Info  where (File_id,Model_Name) in ( ',...
                                     ' select File_id,Model_Name from others_Metric where is_Lib=0 and is_test = -1  and substr(Model_Name,0,length(Model_name)-3) IN (',...
                                     original_mdl_name,') )',...
                                     ' group by BLK_TYPE order by  c desc');
                                 total_hidden_conn_query = strcat('select sum(Conn_count_hidden_only) from others_Hierar_Info where (File_id,Model_Name) in (SELECT distinct File_id,Model_Name from ',...
                                     {' '},choice,{' '},'   where is_Lib = 0 and is_test = -1',...
                                     {' '},' and substr(Model_Name,0,length(Model_name)-3) IN (',original_mdl_name,') )  ');
                           else
        ...
        function res = grand_total_analyze_metric(obj,flag)
            ...
            if flag 
                github = obj.total_analyze_metric('Github_metric',true);
                matc = obj.total_analyze_metric('MATC_metric',true);
            else
                   github = obj.total_analyze_metric('Github_metric',false);
                matc = obj.total_analyze_metric('MATC_metric',false);
            end
            fn = fieldnames(matc);
